...
阶段更新：PLAN-004 已补齐 `Conv2d`、`MaxPool2d`、`Flatten`、`Embedding`、`LayerNorm`、`MultiheadAttention`、`TransformerEncoderLayer`，并新增 MNIST-like CNN 分类和 Transformer 序列分类示例。`softmax` 已作为基础算子暴露；真实 MNIST 下载和缓存策略仍留给后续 DataLoader 专项。
